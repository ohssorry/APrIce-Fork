...
```
이슈 생성 → 브랜치 → 작업 → pytest + ruff → push → `develop` 대상 PR → 리뷰 1명 → merge
```

- **`main`에 직접 push 금지.** 예외 없습니다.
- **모든 PR은 반드시 `develop` 브랜치를 대상으로 올립니다.** `main` 등 다른 브랜치를
  대상으로 PR을 올리지 않습니다.
- **셀프 머지 금지.** 최소 1명 승인 후 머지합니다.
- 브랜치 이름: `feat/…`, `fix/…`, `docs/…`, `test/…`
- **이슈와 PR의 제목 및 본문은 한글로 작성합니다.**
- **커밋 메시지는 한글로 작성합니다.** 접두어 뒤의 설명을 반드시 한글로 씁니다.
...
접두어는 영문으로 유지하되, 콜론 뒤의 커밋 설명은 반드시 한글로 작성합니다.
