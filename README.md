...
aprice diff --base origin/develop --head HEAD --fail-on-risk  # exit 1 on a new structural risk
```
...
`--fail-on-risk` only gates on a *structural* regression: a call moving into a
loop, a loop getting deeper, a known retry bound growing (or no longer being
knowable), or a changed file that stops parsing. A cost increase, a model
swap, or an unpriced call is still reported -- just not gated on, since none
of those is inherently wrong.

## APrIce Guard (this repo's own PRs)

Every pull request against `develop` in this repo gets an APrIce Guard
comment automatically ([`.github/workflows/guard.yml`](.github/workflows/guard.yml)):
cost delta, any new blocking risk, and unpriced calls, generated by
`aprice diff --fail-on-risk` against the PR's base branch. Re-pushing to the
PR updates the same comment (found by an HTML marker) instead of piling up a
new one per push, and the check fails only when a *new* blocking risk is
introduced -- not on cost, model, or token changes alone. On a PR from a
fork, where the workflow token can't post comments, the same report goes to
